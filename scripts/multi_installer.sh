...
# a one line clone-and-compile for bloc:
#
...
_set_wd() {
    if [ -d "$PWD/.git" ] && [ -f "$PWD/Makefile" ]; then
        ...
        if [ -d "$PWD"/bloc ]; then
            read -r -p "${1:-bloc directory already exists. Overwrite? [y/N]} " response
            case "$response" in
                [yY][eE][sS|[yY])
                    _colorize red "Overwriting old bloc directory" && echo
                    rm -rf "$PWD"/bloc
                    ;;
                *)
                    _fail "bloc directory already exists. Aborting..."
                    ;;
            esac
        fi
        mkdir bloc
        git clone -b master -q https://github.com/furiousteam/BLOC bloc   >>build.log 2>&1 || _fail "Unable to clone git repository. Please see build.log for more information"
        cd bloc
    fi
}

_build_bloc() {
    _note "Building bloc from source (this might take a while)..."
    if [ -d build ]; then
...
_configure_linux() {
    if [ "$(awk -F= '/^NAME/{print $2}' /etc/os-release)" = "\"Ubuntu\"" ]; then
        ...
        _fail "Your OS version isn't supported by this installer. Please consider adding support for your OS to the project ('https://github.com/bloc')"
    fi
}
...
_configure_os() {
    ...
    case "${_unameOut}" in
        ...
        Darwin*)
            ...
            ;;
        *)
            _fail "This installer only runs on OSX 10.10+ and Ubuntu 16.04+. Please consider adding support for your OS to the project ('https://github.com/bloc')"
            ;;
    esac
